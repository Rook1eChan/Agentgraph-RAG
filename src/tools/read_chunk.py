...
class ReadChunkTool(BaseTool):
    ...
    def __init__(self, chunks_file: str):
        ...
        self.chunks_dict = {c['title']: c['text'] for c in self.chunks}
...
Note: Previously read chunks will be marked as already seen to avoid redundant information.""",
    ...
    def execute(self, context: 'AgentContext', chunk_ids: List[str] = None, chunk_id: str = None) -> Tuple[str, Dict[str, Any]]:
        ...
        """
        logger.info(f"ReadChunk Input: chunk_ids={chunk_ids}, chunk_id={chunk_id}")
        ...
        if chunk_ids is None:
            if chunk_id is not None:
                ...
                logger.info("ReadChunk Output: Error - No chunk IDs provided")
                return "Error: No chunk IDs provided", {"retrieved_tokens": 0}
        
        chunk_ids = [str(cid) for cid in chunk_ids]
        
        # Convert numeric IDs to full format (e.g., "297" -> "2wikimultihopqa_297")
        # The corpus uses format "2wikimultihopqa_0", "2wikimultihopqa_1", etc.
        dataset_name = "2wikimultihopqa"
        if self.chunks_dict:
            first_key = next(iter(self.chunks_dict.keys()), "")
            if "_" in first_key:
                dataset_name = first_key.rsplit("_", 1)[0]
        
        converted_ids = []
        for cid in chunk_ids:
            if "_" not in cid:
                converted_ids.append(f"{dataset_name}_{cid}")
            else:
                converted_ids.append(cid)
        chunk_ids = converted_ids
        ...
        logger.info(f"ReadChunk Output: new_chunks={len(new_chunks_read)}, already_read={len(already_read)}, retrieved_tokens={total_tokens}")
