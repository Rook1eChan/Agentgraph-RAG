...
class SemanticSearchTool(BaseTool):
    ...
    def _load_index(self):
        ...
        self.index_model_name = index_data.get('model_name', '')
...
RETURNS: Abbreviated snippets with matched sentences. Use read_chunk to get full text for answering.""",
    ...
    def execute(self, context: 'AgentContext', query: str, top_k: int = 5) -> Tuple[str, Dict[str, Any]]:
        logger.info(f"SemanticSearch Input: query={query}, top_k={top_k}")
        ...
        logger.info(f"SemanticSearch Output: chunks_found={len(top_chunks)}, retrieved_tokens={retrieved_tokens}")
